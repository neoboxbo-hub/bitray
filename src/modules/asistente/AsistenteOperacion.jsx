...
export default function AsistenteOperacion() {
  ...
  useEffect(() => {
    localStorage.setItem('bitray.customTokens', JSON.stringify(customTokens))
  }, [customTokens])
  ...
  }, [portfolioMap, cosecha, turbo, customTokens])
  ...
  const handleSelectToken = (sym) => {
    if (sym === '__add_new__') { setShowAddModal(true); return }
  ...
  const handleAddCustomToken = () => {
    ...
    if (!sym) return
    if (!allOptions.find(t => t.symbol === sym)) {
      const nuevo = { symbol: sym, nombre: newNombre.trim() || sym, categoria: 'spec', narrativa: 'Personalizado' }
      setCustomTokens(prev => [...prev, nuevo])
    }
    setShowAddModal(false)
    setNewSymbol('')
    setNewNombre('')
    handleSelectToken(sym)
  }
  ...
  return (
    <div className="space-y-6">
          ...
          <div className="card p-4 space-y-3">
            <div className="grid grid-cols-2 gap-2">
              <div>
                ...
                <input type="number" inputMode="decimal" className="field py-2 text-sm mt-1"
                  placeholder="200" value={capital} onChange={e => setCapital(e.target.value)} />
              </div>
              <div>
            ...
            {precioActual > 0 && (
              <button onClick={() => setEntrada(precioActual.toFixed(6))}
              ...
              <>
                ...
                <div className="space-y-1.5">
                  {resultadoFull?.tps.map(tp => (
                    <div key={tp.pct} className="flex items-center gap-2 bg-profit/10 border border-profit/25 rounded-xl px-3 py-2">
                ...
                <p className="text-[10px] text-gray-600">Neto descontando {TRADING_FEE_PCT}% comisión por lado.</p>
              </>
            )}

            {!hasCalcData && (
              <p className="text-xs text-gray-500 text-center py-2">Ingresa capital y precio de entrada para calcular.</p>
            )}
          </div>
        </section>
      )}
      ...
      <section className="space-y-3">
        ...
        <FearGreedWidget />
        <LiquidationHeatmap />
        <LiquidityPools symbol={selectedSymbol || 'BTC'} precioActual={precioActual || prices['BTC'] || 0} />
      </section>

      {/* ── C) Inteligencia ── */}
      <section className="space-y-3">
        <h2 className="text-xs uppercase tracking-wide text-gray-400 font-semibold px-1">C · Inteligencia y guía personal</h2>
        <AINews />
        <RadarOportunidades />
        <div className="card p-4 space-y-3">
          <div className="flex items-center justify-between">
            <h3 className="font-semibold">📋 Mis reglas de operación</h3>
          ...
          <p className="text-[10px] text-gray-600">Guardado solo en este dispositivo.</p>
        </div>
      </section>

      {/* ── Modal: Agregar token personalizado ── */}
      {showAddModal && (
          ...
          <div className="w-full max-w-md bg-ink-800 rounded-t-3xl p-6 space-y-4 shadow-2xl"
            ...
            <div className="flex items-center justify-between">
              ...
              <button
                ...
                className="h-9 w-9 rounded-xl bg-ink-600 flex items-center justify-center text-gray-300 active:bg-ink-500 text-lg font-bold"
              >
                ✕
              </button>
            </div>
            ...
            <p className="text-sm text-gray-400">
              Agrega cualquier token que no esté en el catálogo (ej: AVAX, SUI, INJ).
            </p>
